...
const staticTagsList = [
	'In-store-shopping', 'Delivery', 'Click & Collect'
]
...
const Homepage = () => {
	...
	useEffect(() => {
		if (!isEmpty(coords) && !isEmpty(option?.tagValue) && option?.tagValue === "Books") {
			...
			<div className="bottom-0 absolute z-[1000] max-h-fit w-[100vw]  flex items-end justify-center  sm:p-0">
				...
				<Transition nodeRef={nodeRef} in={isMenuModalOpen} timeout={duration}>
					{state => (
						...
						<div ref={nodeRef} style={{ ...defaultStyle, ...transitionStyles[state] }} className={cs("w-full   px-4 pb-4 pt-2 mx-auto bg-[#F3F4F5]  rounded-t-[1rem] shadow-lg sm:rounded-lg sm:overflow-hidden")}>
				...
				<div className="flex flex-col gap-2">
					...
					<div className="flex">
						<p className="block  text-[12px] leading-[18px]">
							<span className="font-bold text-ellipsis max-w-[70%]">{selectedStore?.tags.name}</span>	 - Bookstore
						</p>

					</div>
					<div className="flex justify-between gap-2">
						{
							selectedStore?.tags?.image.split(',').map((singleImage: string, i: number) => {
								if (i < 4) return <Image
									src={singleImage}
